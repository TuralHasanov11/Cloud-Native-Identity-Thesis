...
<template>
  <RouterLink v-if="category" :to="`/product-category/${decodeURIComponent(category.id)}`"
    ...
    <img :src="FALLBACK_IMG" :width="imgWidth" :height="imgHeight" class="absolute inset-0 object-cover w-full h-full"
      :alt="category.name" :title="category.name" :sizes="`sm:${imgWidth / 2}px md:${imgWidth}px`"
